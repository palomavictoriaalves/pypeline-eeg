"""
Band × ROI power (ABS/REL), split by EO/EC and PRE/POST.

Inputs (read-only)
- results/processed/sub-XX[/ses-YY]/eeg/
  sub-XX[_ses-YY]_task-<task>[_run-<run>]_desc-preproc_clean_raw.fif
  sub-XX[_ses-YY]_task-<task>[_run-<run>]_desc-preproc_EO_clean_raw.fif (optional)
  sub-XX[_ses-YY]_task-<task>[_run-<run>]_desc-preproc_EC_clean_raw.fif (optional)
...
- results/power/
  ...
  power_wide_rel_EO_EC.csv (if EXPORT_RELATIVE)
  roi_coverage_EO_EC.csv

Notes
- PRE/POST inferred from BIDS session label (ses-pre / ses-post).
- REL power = band / total (within ROI, PSD_FMIN..PSD_FMAX).
"""
...
STD_DUR_SEC = config.STANDARDIZE_DURATION_SEC  # None or seconds
...
_BIDS_CONCAT_RE = re.compile(
    ...
    r"(?:_run-(?P<run>[^_]+))?"
    r"_desc-(?P<desc>[^_]+)"
    r"_clean_raw\.fif$", re.IGNORECASE
)
...
def _open_state_raw(dir_eeg: Path, base: str, state: str):
    ...
    if concat.exists():
        try:
            ...
            return mne.concatenate_raws(pieces, verbose="ERROR") if len(pieces) > 1 else pieces[0]
        except Exception as e:
...
def _manifest_duration(dir_eeg: Path, base: str, state: str) -> float:
    ...
    if not mpath.exists(): return np.nan
    try:
...
# ---- discover concatenated FIFs to define (subject, session, base, dir) ------
pairs = []   # dicts: {"dir": Path, "base": str, "sub": "01", "sess": "PRE|POST"}
seen = set()
...
for f in PROCESSED_DIR.rglob("*_desc-preproc_clean_raw.fif"):
    ...
    if not m:
        continue
    d = m.groupdict()
    base = "_".join([d["sub"]] + ([d["ses"]] if d.get("ses") else []) +
                    [f"task-{d['task']}"] + ([f"run-{d['run']}"] if d.get("run") else []))
    key = (f.parent, base)
    if key in seen:
        continue
    seen.add(key)
...
for item in sorted(pairs, key=lambda x: (x["sub"], x["sess"])):
...
if not rows:
    ...
    if roi_cov_rows:
        ...
        print(f"🧪 ROI coverage saved: {out_cov}")
